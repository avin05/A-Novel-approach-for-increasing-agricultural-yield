...
def Home():
	...
	window.title("CROP PREDICTION AND PLANT LEAF DISEASE DETECTION")
	...
	message1 = tk.Label(window, text="CROP PREDICTION AND PLANT LEAF DISEASE DETECTION" ,bg=bgcolor  ,fg=fgcolor  ,width=50  ,height=2,font=('times', 30, 'italic bold underline')) 
	...
	lbl6 = tk.Label(window, text="SVM RESULT",fg=fgcolor  ,bg=bgcolor ,font=('times', 15, ' bold ') ) 
	lbl6.place(x=70, y=580)
	
	txt6 = tk.Entry(window,bg="white" ,fg="black",font=('times', 15, ' bold '))
	txt6.place(x=200, y=580)

	lbl7 = tk.Label(window, text="SVM Fertilizer",fg=fgcolor  ,bg=bgcolor ,font=('times', 15, ' bold ') ) 
	...
	lbl10 = tk.Label(window, text="Remedy",fg=fgcolor  ,bg=bgcolor ,font=('times', 15, ' bold ') ) 
	lbl10.place(x=730, y=650)

	txt10 = Text(window, bg="white",fg="red",font=('times', 15, ' bold '))
	txt10.place(x=960, y=650,width=200,height=100)
	...
	def predict1():
		...
		if path != "":
			...
			img.image = render
			img.place(x=690, y=140)

			disease = str(remidies[c])
			print(disease)
			txt10.insert(0.0, disease)
